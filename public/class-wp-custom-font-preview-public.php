...
class Wp_Custom_Font_Preview_Public {
    ...
    public function wp_custom_font_preview_input_shortcode($atts) {
		...
		?><div id="wp_custom_font_preview_slider_container">
			<p>
			Adjust the output size:
			</p>
			<input type="range" min="12" value="40" max="80" id="wp_custom_font_preview_slider"/>
			<script>
			jQuery('#wp_custom_font_preview_slider').on('input', function() {
			  var v = $(this).val();
			  $('.wp-custom-font-preview').css('font-size', v + 'px')
			});
			</script>
			</div><?php;
        echo '</div>';
    ...
    public function wp_custom_font_preview_shortcode($atts) {
        ...
        if (! has_shortcode( $post->post_content, 'custom_font_preview_input' ) ) {
            ...
            foreach ($ids as $id) {
				...
				echo '<' . $atts['tag'] . ' class="wp-custom-font-preview-title ' . $atts['class'] . '" style="font-family: ' . $font_name . '" >' . $font_name . '</' . $atts['tag'] . '>';
                echo '<' . $atts['tag'] . ' class="wp-custom-font-preview ' . $atts['class'] . '" style="font-family: ' . $font_name . '"></' . $atts['tag'] . '>';
            }
            echo '</div>';
        }
        return ob_get_clean();
